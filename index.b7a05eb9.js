...
const audioSprite = _utils.loadAudio();
const SIZE = 8;
...
const getTileClickHandler = (tileElement)=>{
    return async ()=>{
        ...
        if (_board.Board.areSwappable(currentTile.tile, tileElement.tile)) {
            ...
            await _utils.playAudio('swap', audioSprite);
            if (board.hasMatches()) do {
                ...
                await _utils.playAudio('pop', audioSprite);
                board.fillUp();
...
const loadAudio = ()=>{
    console.log(Array.from(GAME_AUDIO_URLS.entries()).map(([name, url])=>url.toString()
    ));
    const sound = new _howler.Howl({
        src: [
            new URL(require("c634f38097e441e2")).toString(),
            new URL(require("b10db55713d86307")).toString(), 
        ],
        sprite: {
            pop: [
                0,
...
},{"howler":"5Vjgk","c634f38097e441e2":"h3E1c","b10db55713d86307":"fmzS3","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"5Vjgk":[function(require,module,exports) {
...
},{}],"h3E1c":[function(require,module,exports) {
...
var _delay = require("./delay");
const playAudio = async (sprite, howl)=>{
    ...
    console.log(id, howl.duration(id));
    await _delay.delay(howl.duration(id) * 1500);
};
